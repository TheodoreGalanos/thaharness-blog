...
function predictCardHeight(
  ...
  // date line height + title + gap + description + tags row + padding
  const dateHeight = 20;
  const tagsHeight = 32;
  const innerGaps = 12 + 8 + 12; // gaps between date, title, desc, tags
...
export async function applyMasonryLayout(): Promise<void> {
  ...
  for (const card of cards) {
    ...
    heights.push(predictCardHeight(title, description, cardWidth, 20));
  }
