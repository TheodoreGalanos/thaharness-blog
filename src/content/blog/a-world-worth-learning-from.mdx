...
This time, we changed the sequence: evidence arrived in three releases. The agent had to make a decision and commit before seeing what came next. It then had to preserve decisions that remained valid and revise only those undermined by new evidence. Finally, it had to leave a record behind that another reviewer, human or agent, could continue.
...
I was trying to design long horizon tasks, but I think that by giving the correction up front, I just disguised hindsight as long horizon.
...
To understand how history affects task execution, and how task design shapes its use, I designed four different history carriers:
...
The problem itself and the correct resolution did not change. Only the information carried from one checkpoint to the next changed.
