...
draft: false
pubDate: 2026-04-04
tags: ["recursive-language-models", "harness-engineering", "agent-evaluation", "ai-in-aec"]
featured: true
---
import ProcessComparison from '../../components/charts/ProcessComparison.astro';
import Pullquote from '../../components/Pullquote.astro';
import Sidenote from '../../components/Sidenote.astro';
import { rlmArchitectureShiftData } from '../../data/recursive-by-design-charts';
...
These arguments seem to draw a line. Model on one side. Harness on the other. My goal in this article is to discuss with you why that is more complex than it sounds.
...
Claude Code and Codex are powerful systems. But they still put the full context into the model's context window: the conversation, the documents, the task, all of it. The model attends over all of it, and as the window fills, capability degrades. This is "context rot," and every long-running agent session hits it eventually.
...
What if we computed the decomposition strategy upfront? What if we put domain expertise to use?
...
<ProcessComparison {...rlmArchitectureShiftData} />
...
We started this whole experiment with a model and a REPL. We ended with a deterministic pipeline that costs 14x less, runs in half the time, produces higher-quality output, and tells you what it will cost before it starts. And all we had to do was change the harness.
