...
draft: false
pubDate: 2026-04-03
tags: ["harness-engineering", "agent-evaluation", "ai-in-aec"]
featured: true
---

import Figure from '../../components/Figure.astro';
import aecBenchTui from '../../assets/blog/aec-bench/aec-bench-tui.png';
...
But when agents were equipped with domain-specific document parsing and retrieval, performance jumped dramatically. The gains were twenty to thirty points on the hardest task families, far exceeding what any model upgrade alone could deliver.
...
AEC-Bench is a start, but it is only a start. The industry needs benchmarks that evolve — that grow with more task families, more disciplines, and document types the current set doesn't cover. It needs evaluation infrastructure that supports reproducibility, trajectory analysis, and systematic comparison of harness designs. It also needs tooling that lets you understand *how* an agent succeeded or failed, and what that means for the next iteration.

This is what **aec-bench** is supposed to be — an open platform for AEC agent evaluation — and we will have more to share soon.

<Figure
	src={aecBenchTui}
	alt="AEC-Bench terminal interface showing task generation, experiment orchestration, evaluation, and harness comparison"
	caption="aec-bench: from task templates and instance generation to experiment orchestration, evaluation, and comparison across harness designs — all from the terminal."
/>

In the meantime, the [paper](https://arxiv.org/abs/2603.29199) and benchmark [dataset](https://huggingface.co/datasets/nomic-ai/aec-bench) are available now. If you're interested in making AI work in the built environment, take a look and share your thoughts! And if you're still relying on anecdotes to judge what these systems can do, consider that you might be optimising for demos when you should be optimising for deployment.
