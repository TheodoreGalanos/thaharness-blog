...
Caution in prose is not caution in the record. The strong models say the stale dashboard or cached pass cannot govern, then fail to preserve the exact rejected evidence, governing evidence, or threshold that proves it. A reviewer inspects the record rather than the intent. Status codes are part of that proof, not wording preferences; *hold* and *limited* sound equally careful and route to different downstream handling. Governance fields (evidence IDs, thresholds, owners, action and status codes) are first-class outputs rather than formatting details: the policy was not followed unless the record says why.
...
A summary is context, not authority. The partial-dashboard result is the good news: the strong models reliably refused to let an aggregate dashboard stand in for shards it never sampled. The authority boundary held under load. But closeout can be correct and still unauditable. The models almost always get the ready-or-hold verdict right; what they drop is the trail underneath it — which required dependency failed, which result was used, which was rejected, which blocker applies. That trail is the proof, not the verdict.
