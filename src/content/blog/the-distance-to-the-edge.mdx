...
seoTitle: "Organisational AI Capability and Learning Rate | The Distance to the Edge"
...
It turns out that distances are not evenly distributed.
...
Our goal has been to stay close enough to the frontier to encounter new capabilities early and build the intuition required to understand what they meant. We aimed to test them quickly against real domain problems, reject what did not work, try to operationalise what did, and carry the learning into the next cycle.
...
An organisation begins by becoming aware of AI. It then experiments, pilots, deploys, scales, and eventually transforms. Each stage appears to be another step along the same path. We like this description because it is simple and straightforward, but I think it hides an important phase change.
...
Two organisations are not separated only by what they have built, but also by what they have had to learn in order to build reliably.
...
From experience, that is a difficult thing to balance: exploration, speed, governance, and scale. Staying at the edge requires all of these to be present, with the right amount of tension between them to remain active.
