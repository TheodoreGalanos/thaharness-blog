...
<GroupedBars {...bondsComparisonData} />
...
<BarsWithBaseline {...session2TrajectoryData} />
...
- **Iteration 3** restructured the workflow to a two-pass approach: sweep all totals first, then deep-dive flagged rooms. No reward improvement, and token use nearly doubled — 79 turns and 1.08 million tokens. It mostly shuffled which errors were caught without improving overall coverage.
...
<ConceptGrid {...strategyContrastData} />
...
<BarsWithBaseline {...session3L0TrajectoryData} />
...
<BarsWithBaseline {...session4Gpt41miniTrajectoryData} />
...
<GroupedBars {...crossModelBondsL0Data} />
...
For now, the result is small and specific. We have an autonomous loop, an information barrier, and behavioural feedback that inverted our assumptions about verification. One prompt change helped at full reference; another worked for a very different reason at `L0`. A cross-model comparison showed that prompt ideas can transfer even when prompt wording does not. The environment was improvable in each case, but the successful strategy depended on what information the environment already supplied and which model was inside it. The agents that scored best were not simply the ones that checked the most. They were the ones whose workflow matched the structure of the task, the information available, and the model's own behavioural constraints.
