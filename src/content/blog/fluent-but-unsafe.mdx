...
The agent must produce the full review record. It must record which documents it received and cross-check that they all refer to the same road, location, equipment, and design scenario. It must provide one status for each review item, its own recomputed evidence, findings and requests for missing information, actions with owners, and a final decision on whether the work is ready to issue.
...
For this first set of model runs, each summary recorded the random seed and exact code version, so I could recreate the task; I stored the full model responses separately. That was enough to debug these tasks. In later experiments (part 2 of this series), evidence will arrive over time. Those experiments need permanent records of exactly how every run was produced, plus a clear separation between runs used to tune the setup and unseen tasks used only for final evaluation.
